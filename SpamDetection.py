...
main = tkinter.Tk()
main.title("Spammer detection and fake user identification on social network") #designing main screen
      ...
      for fdata in files:
        with open(root+"/"+fdata, "r") as file:
            ...
            if followers < following:
                text.insert(END,"social  Account is Fake\n")
                fake = 1
                fake_acc = fake_acc + 1
            else:
                text.insert(END,"social  Account is Genuine\n")
                fake = 0
            text.insert(END,"\n")
...
def graph():
    ...
    bars = ('Total social Accounts', 'Fake Accounts','Spam Content Tweets')
...
font = ('times', 16, 'bold')
title = Label(main, text='Spammer detection and fake user identification on social network')
...
font1 = ('times', 14, 'bold')
uploadButton = Button(main, text="Upload social  JSON Format Tweets Dataset", command=upload)
